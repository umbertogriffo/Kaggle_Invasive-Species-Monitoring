...
for img_path in sorted(os.listdir(train_data_dir),key=lambda x: (int(re.sub('\D','',x)),x)): # sort by key
    img = image.load_img(train_data_dir+'/'+img_path,target_size=(190, 256))
...
for img_path in sorted(os.listdir(test_data_dir),key=lambda x: (int(re.sub('\D','',x)),x)): # sort by key
    img = image.load_img(test_data_dir+'/'+img_path,target_size=(190, 256))
...
def model_nn():
    # Create the model
    model = Sequential()
    model.add(Conv2D(16, (3, 3), activation='relu', input_shape=(3,190, 256)))
